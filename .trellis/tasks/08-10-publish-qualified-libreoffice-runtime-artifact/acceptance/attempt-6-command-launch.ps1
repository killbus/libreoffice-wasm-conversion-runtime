...
function Resolve-Attempt5Application {
...
function ConvertTo-Attempt5CmdToken {
...
function Set-Attempt5ProcessCommand {
  param(
  ...
  if ($extension -ieq '.cmd' -or $extension -ieq '.bat') {
    ...
    $cmdTokens = @((ConvertTo-Attempt5CmdToken $ResolvedExecutable))
    $cmdTokens += @($Arguments | ForEach-Object { ConvertTo-Attempt5CmdToken $_ })
