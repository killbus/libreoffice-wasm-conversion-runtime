...
param(
  ...
  [string]$AcceptanceRoot = 'D:\tmp\lo-runtime-acceptance-attempt-5'
)
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'Acceptance Attempt 5 requires PowerShell 7 or later (pwsh).'
}

$TimeContractPath = Join-Path $PSScriptRoot 'attempt-5-time-contract.ps1'
if (-not (Test-Path -LiteralPath $TimeContractPath -PathType Leaf)) {
  throw "Acceptance Attempt 5 timestamp contract helper is missing: $TimeContractPath"
}
. $TimeContractPath

$CommandLaunchContractPath = Join-Path $PSScriptRoot 'attempt-5-command-launch.ps1'
if (-not (Test-Path -LiteralPath $CommandLaunchContractPath -PathType Leaf)) {
  throw "Acceptance Attempt 5 command-launch contract helper is missing: $CommandLaunchContractPath"
}
...
$CommandLaunchContractTestPath = Join-Path $PSScriptRoot 'attempt-5-command-launch.tests.ps1'
...
$downloadHelper = Join-Path $PSScriptRoot 'attempt-5-download-assets.mjs'
