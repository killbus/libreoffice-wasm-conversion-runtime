...
function Resolve-Attempt7Application {
...
function ConvertTo-Attempt7CmdToken {
...
function Set-Attempt7ProcessCommand {
  param(
  ...
  if ($extension -ieq '.cmd' -or $extension -ieq '.bat') {
    ...
    $cmdTokens = @((ConvertTo-Attempt7CmdToken $ResolvedExecutable))
    $cmdTokens += @($Arguments | ForEach-Object { ConvertTo-Attempt7CmdToken $_ })
