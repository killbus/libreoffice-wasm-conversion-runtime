...
$HelperPath = Join-Path $PSScriptRoot 'attempt-8-command-launch.ps1'
. $HelperPath
...
function Invoke-ConfiguredProcess {
  ...
  $contract = Set-Attempt8ProcessCommand -StartInfo $startInfo -ResolvedExecutable $Executable -Arguments $Arguments
...
$testRoot = Join-Path $tempBase ("lo-runtime-attempt-8-command-launch-{0} with spaces" -f [Guid]::NewGuid().ToString('N'))
...
Assert-Contract ([IO.Path]::GetFileName($testRoot).StartsWith('lo-runtime-attempt-8-command-launch-', [StringComparison]::Ordinal)) 'Unexpected test-root leaf name.'
New-Item -ItemType Directory -Path $testRoot -ErrorAction Stop | Out-Null

try {
  $captureJs = Join-Path $testRoot 'capture-args.cjs'
  [IO.File]::WriteAllText($captureJs, "const fs = require('node:fs');`nfs.writeFileSync(process.env.ATTEMPT8_CAPTURE, JSON.stringify(process.argv.slice(2)));`n", [Text.UTF8Encoding]::new($false))
  ...
  foreach ($wrapper in @($cmdWrapper, $batWrapper)) {
    ...
    $result = Invoke-ConfiguredProcess -Executable $wrapper -Arguments $expectedArguments -WorkingDirectory $testRoot -Environment @{ ATTEMPT8_CAPTURE = $capturePath }
  ...
  if ($resolvedRoot.StartsWith($tempBase, [StringComparison]::OrdinalIgnoreCase) -and [IO.Path]::GetFileName($resolvedRoot).StartsWith('lo-runtime-attempt-8-command-launch-', [StringComparison]::Ordinal)) {
