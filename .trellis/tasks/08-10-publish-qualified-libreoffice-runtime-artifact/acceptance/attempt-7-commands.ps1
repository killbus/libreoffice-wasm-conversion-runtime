...
param(
  ...
  [string]$AcceptanceRoot = 'D:\tmp\lo-runtime-acceptance-attempt-7'
)
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'Acceptance Attempt 7 requires PowerShell 7 or later (pwsh).'
}

$TimeContractPath = Join-Path $PSScriptRoot 'attempt-7-time-contract.ps1'
if (-not (Test-Path -LiteralPath $TimeContractPath -PathType Leaf)) {
  throw "Acceptance Attempt 7 timestamp contract helper is missing: $TimeContractPath"
}
. $TimeContractPath

$CommandLaunchContractPath = Join-Path $PSScriptRoot 'attempt-7-command-launch.ps1'
if (-not (Test-Path -LiteralPath $CommandLaunchContractPath -PathType Leaf)) {
  throw "Acceptance Attempt 7 command-launch contract helper is missing: $CommandLaunchContractPath"
}
. $CommandLaunchContractPath
...
$ExtractRoot = Join-Path $AcceptanceRoot 'release-extract'
$PreparedCandidateRoot = Join-Path $PdfHowRepository 'third_party\libreoffice-wasm-conversion-runtime-dev'
$PreparationContractWork = Join-Path $EvidenceRoot 'preparation-contract-work'
...
function Invoke-FailClosedCommand {
  ...
  $resolvedExecutable = Resolve-Attempt7Application $Executable
  ...
  $launchContract = Set-Attempt7ProcessCommand -StartInfo $startInfo -ResolvedExecutable $resolvedExecutable -Arguments $Arguments
  $environment = [ordered]@{}
...
$env:OFFICE_RUNTIME_ROOT = $PreparedCandidateRoot
...
$CommandLaunchContractTestPath = Join-Path $PSScriptRoot 'attempt-7-command-launch.tests.ps1'
Invoke-FailClosedCommand 'windows-cmd-bat-command-launch-contract' 'pwsh' @(
  '-NoLogo',
  '-NoProfile',
  '-File',
  $CommandLaunchContractTestPath
) $PSScriptRoot 120 | Out-Null
$TimeContractTestPath = Join-Path $PSScriptRoot 'attempt-7-time-contract.tests.ps1'
Invoke-FailClosedCommand 'powershell-json-timestamp-contract' 'pwsh' @(
  '-NoLogo',
  '-NoProfile',
  '-File',
  $TimeContractTestPath
) $PSScriptRoot 120 | Out-Null

# Fresh, detached, pinned repository checkouts in the new Attempt 7 root.
...
Invoke-FailClosedCommand 'checkout-runtime' 'git' @('checkout', '--detach', $RuntimeCommit) $RuntimeRepository 300 | Out-Null
...
Invoke-FailClosedCommand 'clone-pdfhow' 'git' @('clone', '--no-checkout', $PdfHowRepositoryUrl, $PdfHowRepository) $AcceptanceRoot 900 | Out-Null
Invoke-FailClosedCommand 'checkout-pdfhow' 'git' @('checkout', '--detach', $PdfHowCommit) $PdfHowRepository 300 | Out-Null
...
$downloadHelper = Join-Path $PSScriptRoot 'attempt-7-download-assets.mjs'
...
# Exercise the exact fixed-checkout PDFHow preparation contract using isolated
# synthetic inputs before preparing downloaded bytes. This test proves that the
# helper creates all three control files and fails closed on identity/schema
# mutation or a renamed Release manifest. It does not run Chromium/conversion.
$PreparationContractTest = Join-Path $PSScriptRoot 'attempt-7-prepare-candidate.tests.mjs'
Assert-True (Test-Path -LiteralPath $PreparationContractTest -PathType Leaf) "Preparation contract test missing: $PreparationContractTest"
Invoke-FailClosedCommand 'pdfhow-preparation-helper-contract' 'node' @(
  $PreparationContractTest,
  '--pdfhow-repository', $PdfHowRepository,
  '--work-root', $PreparationContractWork,
  '--report-out', (Join-Path $EvidenceRoot 'preparation-contract-test.json')
) $PSScriptRoot 300 | Out-Null

# Convert the verified Release extraction into PDFHow's required local candidate
# workspace by invoking the official helper from the fixed PDFHow checkout.
# Copying or renaming CANDIDATE-MANIFEST.json is explicitly forbidden.
$PreparationHelper = Join-Path $PdfHowRepository 'scripts/prepare-libreoffice-runtime-candidate.mjs'
Assert-True (Test-Path -LiteralPath $PreparationHelper -PathType Leaf) "Fixed PDFHow preparation helper missing: $PreparationHelper"
Invoke-FailClosedCommand 'prepare-pdfhow-local-candidate-workspace' 'node' @(
  'scripts/prepare-libreoffice-runtime-candidate.mjs',
  '--native-root', (Join-Path $ExtractRoot 'wasm'),
  '--wrapper-root', $ExtractRoot,
  '--destination', $PreparedCandidateRoot,
  '--native-commit', $NativeCommit,
  '--wrapper-commit', $ReleaseTarget,
  '--github-actions-run-id', ([string]$NativeWorkflowRunId),
  '--native-abi', 'lok-convert-document-v1',
  '--native-schema-version', '1',
  '--pthread-worker-mode', 'main-script'
) $PdfHowRepository 900 | Out-Null

$PreparedCandidateVerifier = Join-Path $PSScriptRoot 'attempt-7-verify-prepared-candidate.mjs'
Assert-True (Test-Path -LiteralPath $PreparedCandidateVerifier -PathType Leaf) "Prepared candidate verifier missing: $PreparedCandidateVerifier"
$preparedCandidateCheck = Invoke-FailClosedCommand 'verify-pdfhow-local-candidate-workspace' 'node' @(
  $PreparedCandidateVerifier,
  '--candidate-root', $PreparedCandidateRoot,
  '--expected-candidate-id', $CandidateId,
  '--expected-native-root', (Join-Path $ExtractRoot 'wasm'),
  '--expected-wrapper-root', $ExtractRoot,
  '--expected-native-commit', $NativeCommit,
  '--expected-wrapper-commit', $ReleaseTarget,
  '--expected-run-id', ([string]$NativeWorkflowRunId),
  '--expected-abi', 'lok-convert-document-v1',
  '--expected-native-schema-version', '1',
  '--expected-pthread-worker-mode', 'main-script'
) $PSScriptRoot 300
$preparedCandidate = $preparedCandidateCheck.stdout | ConvertFrom-Json
Assert-True ($preparedCandidate.passed -eq $true) 'Prepared candidate verifier did not report passed.'
Assert-True ($preparedCandidate.candidateId -eq $CandidateId) 'Prepared candidate verifier reported a different candidate ID.'
Assert-True ($preparedCandidate.releaseQualified -eq $false) 'Prepared candidate must remain releaseQualified: false.'
Write-JsonEvidence (Join-Path $EvidenceRoot 'prepared-candidate-assertions.json') ([ordered]@{
  passed = $true
  officialPreparationHelper = 'scripts/prepare-libreoffice-runtime-candidate.mjs'
  simpleManifestCopyOrRenameUsed = $false
  candidateRoot = $PreparedCandidateRoot
  requiredControlFiles = @('LOCAL-CANDIDATE-METADATA.json', 'SHA256SUMS', 'package.json')
  exactSources = @('nativeRoot', 'wrapperRoot')
  exactRuntimeAssets = $ExpectedRuntimeFiles
  candidateId = $CandidateId
  abi = 'lok-convert-document-v1'
  schemaVersion = 1
  pthreadWorkerMode = 'main-script'
  releaseQualified = $false
  inventory = $preparedCandidate.inventory
})
...
Write-JsonEvidence (Join-Path $AcceptanceRoot 'ATTEMPT-7-COMMAND-COMPLETED.json') ([ordered]@{
  acceptanceAttempt = 7
...
Write-Host "Acceptance Attempt 7 command sequence completed without command failure. Evidence: $EvidenceRoot"
