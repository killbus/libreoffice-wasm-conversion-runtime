...
param(
  ...
  [string]$AcceptanceRoot = 'D:\tmp\lo-runtime-acceptance-attempt-4'
)
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'Acceptance Attempt 4 requires PowerShell 7 or later (pwsh).'
}

$TimeContractPath = Join-Path $PSScriptRoot 'attempt-4-time-contract.ps1'
if (-not (Test-Path -LiteralPath $TimeContractPath -PathType Leaf)) {
  throw "Acceptance Attempt 4 timestamp contract helper is missing: $TimeContractPath"
}
. $TimeContractPath
...
# Fresh, detached, pinned repository checkouts in the new Attempt 4 root.
...
$downloadHelper = Join-Path $PSScriptRoot 'attempt-4-download-assets.mjs'
...
Write-JsonEvidence (Join-Path $AcceptanceRoot 'ATTEMPT-4-COMMAND-COMPLETED.json') ([ordered]@{
  acceptanceAttempt = 4
...
Write-Host "Acceptance Attempt 4 command sequence completed without command failure. Evidence: $EvidenceRoot"
