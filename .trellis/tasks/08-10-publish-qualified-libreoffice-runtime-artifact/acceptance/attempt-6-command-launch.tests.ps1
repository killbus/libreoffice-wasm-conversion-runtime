...
$HelperPath = Join-Path $PSScriptRoot 'attempt-6-command-launch.ps1'
. $HelperPath
...
function Invoke-ConfiguredProcess {
  ...
  $contract = Set-Attempt5ProcessCommand -StartInfo $startInfo -ResolvedExecutable $Executable -Arguments $Arguments
...
$testRoot = Join-Path $tempBase ("lo-runtime-attempt-6-command-launch-{0} with spaces" -f [Guid]::NewGuid().ToString('N'))
...
Assert-Contract ([IO.Path]::GetFileName($testRoot).StartsWith('lo-runtime-attempt-6-command-launch-', [StringComparison]::Ordinal)) 'Unexpected test-root leaf name.'
...
try {
  ...
  if ($resolvedRoot.StartsWith($tempBase, [StringComparison]::OrdinalIgnoreCase) -and [IO.Path]::GetFileName($resolvedRoot).StartsWith('lo-runtime-attempt-6-command-launch-', [StringComparison]::Ordinal)) {
