...
import { verifyPreparedCandidate } from './attempt-7-verify-prepared-candidate.mjs'
...
async function main() {
  ...
  const workRoot = args['work-root'] ? resolve(args['work-root']) : await mkdtemp(join(tmpdir(), 'attempt-7-prepare-contract-'))
  ...
  for (const [rootKind, path] of ASSETS) {
    ...
    await writeFile(target, `attempt-7 contract fixture: ${rootKind}/${path}\n`, 'utf8')
  }
  ...
  const report = {
    schemaVersion: 1,
    kind: 'acceptance-attempt-7-preparation-contract-test',
    passed: true,
...
main().catch(error => {
  console.error(`[attempt-7-prepare-candidate-tests] ${error instanceof Error ? error.message : String(error)}`)
  process.exitCode = 1
})