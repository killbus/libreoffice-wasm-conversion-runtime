...
param(
  ...
  [string]$AcceptanceRoot = 'D:\tmp\lo-runtime-acceptance-attempt-6'
)
...
if ($PSVersionTable.PSVersion.Major -lt 7) {
  throw 'Acceptance Attempt 6 requires PowerShell 7 or later (pwsh).'
}

$TimeContractPath = Join-Path $PSScriptRoot 'attempt-6-time-contract.ps1'
if (-not (Test-Path -LiteralPath $TimeContractPath -PathType Leaf)) {
  throw "Acceptance Attempt 6 timestamp contract helper is missing: $TimeContractPath"
}
. $TimeContractPath

$CommandLaunchContractPath = Join-Path $PSScriptRoot 'attempt-6-command-launch.ps1'
if (-not (Test-Path -LiteralPath $CommandLaunchContractPath -PathType Leaf)) {
  throw "Acceptance Attempt 6 command-launch contract helper is missing: $CommandLaunchContractPath"
}
...
function Invoke-FailClosedCommand {
  ...
  $resolvedExecutable = Resolve-Attempt5Application $Executable
  ...
  $launchContract = Set-Attempt5ProcessCommand -StartInfo $startInfo -ResolvedExecutable $resolvedExecutable -Arguments $Arguments
  $environment = [ordered]@{}
...
$CommandLaunchContractTestPath = Join-Path $PSScriptRoot 'attempt-6-command-launch.tests.ps1'
...
# Fresh, detached, pinned repository checkouts in the new Attempt 5 root.
...
$downloadHelper = Join-Path $PSScriptRoot 'attempt-6-download-assets.mjs'
...
Write-JsonEvidence (Join-Path $AcceptanceRoot 'ATTEMPT-5-COMMAND-COMPLETED.json') ([ordered]@{
  acceptanceAttempt = 5
...
Write-Host "Acceptance Attempt 5 command sequence completed without command failure. Evidence: $EvidenceRoot"
