...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  try {
    ...
    process.stdout.write(`${JSON.stringify({ schemaVersion: 1, kind: 'acceptance-attempt-7-prepared-candidate-verification', passed: true, candidateRoot: result.candidateRoot, candidateId: result.candidateId, releaseQualified: result.releaseQualified, inventory: result.inventory, assets: result.assets }, null, 2)}\n`)
  } catch (error) {
    console.error(`[attempt-7-verify-prepared-candidate] ${error instanceof Error ? error.message : String(error)}`)
    process.exitCode = 1
  }
}