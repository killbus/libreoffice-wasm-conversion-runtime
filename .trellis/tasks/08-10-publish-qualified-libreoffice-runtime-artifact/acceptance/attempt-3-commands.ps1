...
param(
  ...
  [string]$AcceptanceRoot = 'D:\tmp\lo-runtime-acceptance-attempt-3'
)
...
# Fresh, detached, pinned repository checkouts in the new Attempt 3 root.
...
$downloadHelper = Join-Path $PSScriptRoot 'attempt-3-download-assets.mjs'
...
Write-JsonEvidence (Join-Path $AcceptanceRoot 'ATTEMPT-3-COMMAND-COMPLETED.json') ([ordered]@{
  acceptanceAttempt = 3
...
Write-Host "Acceptance Attempt 3 command sequence completed without command failure. Evidence: $EvidenceRoot"
