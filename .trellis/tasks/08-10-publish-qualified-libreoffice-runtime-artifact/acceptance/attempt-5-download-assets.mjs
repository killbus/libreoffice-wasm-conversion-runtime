...
async function main() {
  ...
  if (!outputRootArg || process.argv.length !== 3) fail('Usage: node attempt-5-download-assets.mjs <fresh-output-directory>')
  ...
  if (release.id !== RELEASE_ID || release.tag_name !== TAG || release.target_commitish !== TARGET || release.draft !== true || release.published_at !== null || release.assets?.length !== EXPECTED_ASSETS.length) fail('GitHub Release identity/state differs from the frozen Attempt 5 handoff')
  ...
  const report = { schemaVersion: 1, kind: 'acceptance-attempt-5-release-download', generatedAt: new Date().toISOString(), retryPerformed: false, release: { id: release.id, url: release.html_url, tag: release.tag_name, target: release.target_commitish, draft: release.draft, publishedAt: release.published_at, updatedAt: release.updated_at }, candidateId: CANDIDATE_ID, releaseQualified: false, assets: downloadedAssets }
  await writeFile(resolve(outputRoot, 'attempt-5-release-download.json'), `${JSON.stringify(report, null, 2)}\n`, { encoding: 'utf8', flag: 'wx' }); process.stdout.write(`${JSON.stringify(report, null, 2)}\n`)
}
main().catch((error) => { console.error(`[attempt-5-download-assets] ${error.message}`); process.exitCode = 1 })
