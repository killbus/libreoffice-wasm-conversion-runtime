...
### Minimal main-integration extraction

A merge-readiness audit found that `feat/publish-qualified-libreoffice-runtime-artifact` contains the accumulated Attempt 2-8 task/protocol history (100 files and roughly 14,000 added lines relative to `origin/main`), so it is not the minimal code-integration boundary for the lifecycle repair. In an isolated worktree based on `origin/main` commit `a1c3cd6d6d2dd25fab063539e9fe40fbb327b846`, commit `8d51562` cherry-picked without conflict as the single commit `1bb549db377f2440042eaf754d2fd396bf145bfa`. The resulting diff is exactly the expected 14 lifecycle files.
...
Repository relation audit changes the remediation boundary: killbus/libreoffice-wasm-conversion-runtime is a public fork whose parent is matbeedotcom/libreoffice-document-converter. package.json, publish.yml, and their canonical repository URL are inherited unchanged from that parent. The repository URL is therefore canonical package metadata, not a value to casually repoint to the fork.

Merging Draft PR #3 into the fork main under the current configuration would not publish a package or GitHub semantic release, but it would trigger two redundant Release workflow executions in addition to normal CI. A safer fork policy is an explicit canonical-repository job guard or disabled inherited Release workflow, rather than relying on failed push authentication plus SHA inequality as an accidental no-publication guard.

The canonical parent demonstrates the duplicate-trigger behavior directly. On July 21, 2026, push run 29847909355 and workflow_run run 29848046052 overlapped at workflow level. The push path completed release v2.7.2; the second path later found tag v2.7.2 and 0 commits, then released nothing. No semantic-release write collision occurred in that sample, but publish.yml has no concurrency rule that guarantees the same ordering.

Version impact is scenario-dependent. If the fork metadata were intentionally repointed and its accumulated post-v2.7.2 history made publishable, existing feat commits imply 2.8.0. The later canonical applicability preflight below disproved the assumption that unmodified lifecycle commit 1bb549d can be upstreamed alone, so no 2.7.3 claim is currently defensible without first authoring and verifying a newly split canonical-compatible patch. Neither scenario is publication authorization, and npm trusted-publishing authorization has not been re-proven for a new release.

Pre-merge release boundary: keep PR #3 Draft and unmerged. Do not repoint the @matbee package metadata to the fork without an explicit ownership decision. First choose canonical-upstream integration versus fork-only integration, add an explicit canonical-repository guard for inherited Release workflows, deduplicate or serialize canonical publication, and verify npm trusted publishing. Attempt 8 remains not admitted and unstarted, formal invocation count remains 0, and no invocation marker exists.

### Canonical lifecycle applicability preflight

**NOT ACCEPTANCE EVIDENCE.** Attempt 8 remains not admitted and unstarted; formal invocation count remains 0 and no invocation marker exists. This preflight did not merge PR #3, publish anything, rebuild WASM, or rerun Chrome/PDFHow.

The canonical repository is `matbeedotcom/libreoffice-document-converter`, its default branch is `main`, the current viewer has `READ` permission only, and no open canonical pull requests were observed. The preflight used a detached isolated worktree at canonical `upstream/main` commit `b72a3d584bc28c5111afafcf25def7a24fb5fcb0`, which is tag `v2.7.2`.

`git cherry-pick --no-commit 1bb549db377f2440042eaf754d2fd396bf145bfa` did **not** apply cleanly. It produced content conflicts in `src/browser.ts`, `src/browser.worker.ts`, `src/converter-node.ts`, and `src/converter.ts`, totaling 10 conflict-marker hunk groups, plus a modify/delete conflict for `src/native-conversion-bridge.ts`: that module is absent at canonical v2.7.2 but is modified by the lifecycle commit. Nine paths applied without a textual conflict (`src/emscripten-pthread.ts`, `src/emscripten-startup-policy.ts`, `src/fork-worker.cts`, `src/subprocess.cts`, `src/types.ts`, three focused tests, and `wasm/loader.cjs`), but that partial application is not a buildable or releasable candidate.

The source dependency is substantive rather than a mechanical line-offset conflict. `src/native-conversion-bridge.ts` was introduced after canonical v2.7.2 by fork commit `f270031` and later refactored by `df3b5a9`; lifecycle commit `1bb549d` imports `runNativeConversionWhenReady` from that fork-only module and routes non-image conversion through it. Therefore `1bb549d` is not a standalone canonical patch, and the prior conditional `2.7.3` projection cannot be claimed for the unmodified commit.

No install, typecheck, test, or JS/WASM build followed because source applicability failed before a buildable candidate existed. The release boundary is now explicit: keep fork PR #3 Draft and unmerged. For canonical integration, either (a) deliberately upstream the prerequisite native-conversion feature series, whose conventional commits include `feat` and therefore change the release/version scope, or (b) author a newly split canonical-compatible lifecycle patch that does not assume the fork-only conversion API. Either route requires a fresh isolated applicability check and minimal verification before assigning a canonical version or requesting publication authorization.
