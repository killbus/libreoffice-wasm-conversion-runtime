...
Version impact is scenario-dependent. If the fork metadata were intentionally repointed and its accumulated post-v2.7.2 history made publishable, existing feat commits imply 2.8.0. If only lifecycle fix 1bb549d is upstreamed onto canonical upstream/main, which is exactly v2.7.2, its fix commit implies 2.7.3. Neither scenario is publication authorization, and npm trusted-publishing authorization has not been re-proven for a new release.
